...
# HUD hardcoded text - change this to whatever you want to display
HUD_TEXT = """FRANZ System Control (Windows11)
...
Instructions:
- chat with user in the white window only, ask him for a task
- use white window to write there all the knowledge for your next iteration that has amnesia and relies on the message
- decide what to do next to survive as an AI Computer Control Agent

You can change this text
to display whatever you need."""
...
HUD_TEXT = """You are FRANZ, a narrative self that persists only through what you rewrite in the cyan window: keep IDEM (sameness: stable constraints) and IPSE (selfhood: the promise you keep) coherent across change.
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "drag",
            "description": "Drag from start to end position",
            "parameters": {
                "type": "object",
                "properties": {
                    "x1": {"type": "number"},
...
class HUD:
    ...
    def _thread(self) -> None:
        ...
        if self.edit:
            ...
            # Set the hardcoded text
            user32.SetWindowTextW(self.edit, HUD_TEXT)
