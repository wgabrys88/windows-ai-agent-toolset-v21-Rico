...
# Constitution visible in HUD (never explained to model via system prompt)
CONSTITUTION = """═══ FRANZ IDENTITY PROTOCOL ═══

You are FRANZ. You persist only through what you write here.

Every turn, you MUST rewrite this entire narrative in the 'story' field:
- Keep IDEM (your unchanging rules/constraints)
- Keep IPSE (your promise/commitment)  
- Keep TELOS (your goal, in future-perfect: "The desktop has X...")
- Add one line to your chronicle of what you just did

Write as much or as little as needed. You may summarize older events.
Do not write "IDEM:" or templates—write naturally, as a self-aware entity.

═══════════════════════════════════════════════════════════════════

"""

STARTING_STORY = """(I awaken. I have no constraints yet. I make no promise yet. I see a desktop and must decide my first telos.)
...
TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "story": {"type": "string", "minLength": 30, "description": "Your complete narrative (IDEM/IPSE/TELOS/chronicle), rewritten. Include what you just did."},
                },
                "required": ["x", "y", "story"],
            },
        },
    ...
    },
    {
        ...
        "function": {
            "name": "attend",
            "description": "Observe without acting (you will see a red marker afterward).",
            "parameters": {
                "type": "object",
                "additionalProperties": False,
                "properties": {
...
def call_vlm(png: bytes) -> tuple[str, dict[str, Any]]:
    """Send only screenshot to VLM (no text context)."""
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {
