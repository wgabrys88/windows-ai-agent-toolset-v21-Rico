...
def normalize_args(args: Any) -> dict[str, Any]:
    """Best-effort normalization of VLM tool arguments."""
    ...
    # Fuzzy match missing numeric args
...
def capture_screen(sw: int, sh: int) -> bytes:
    ...
    # CAPTUREBLT ensures layered windows are captured
...
def _parse_tool_arguments(args_raw: Any) -> dict[str, Any]:
    ...
    # Try plain JSON, then handle double-encoded strings
    for _ in range(2):
        try:
            parsed = json.loads(s)
    ...
    # Salvage first {...} if extra text present
    ...
    def _wndproc(self, hwnd: w.HWND, msg: int, wparam: w.WPARAM, lparam: w.LPARAM) -> w.LPARAM:
        try:
            if msg == WM_COMMAND and (int(wparam) & 0xFFFF) == self._BTN_ID:
                ...
                return 0
            elif msg == WM_MOUSEWHEEL and user32.GetAsyncKeyState(0x11) & 0x8000:  # Ctrl held
...
def main() -> None:
    ...
    with HUD() as hud:
        ...
        current_story = full_text.replace(STORY_PREAMBLE, "").strip() if full_text and full_text != STORY_PREAMBLE.strip() else ""
        ...
        while not hud.stop.is_set():
            ...
            # Hide previous observation window after it's been captured
            if obs:
                obs.hide()
                obs = None
