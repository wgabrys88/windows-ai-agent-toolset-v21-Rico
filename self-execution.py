...
STORY_FILE = DUMP_FOLDER / "franz_memory.txt"
...
2. Choose one action.
3. In the story field, rewrite your complete narrative:
...
You run as main.py. You can edit your code, schedule restart, commit suicide.
...
TOOLS = [
    {
        ...
        "function": {
            "name": "click",
            "description": "Click at coordinates. 0,0=top-left, 1000,1000=bottom-right.",
            "parameters": {
                "type": "object",
...
class MSG(ctypes.Structure):
    _fields_ = [
        ("hwnd", w.HWND), ("message", ctypes.c_uint),
...
class WNDCLASSEXW(ctypes.Structure):
    _fields_ = [
        ...
        ("hIcon", w.HANDLE), ("hCursor", w.HANDLE),
...
class HUD:
    ...
    def _layout(self) -> None:
        ...
        pad, bh, bw = 10, 50, min(170, max(80, cw - 20))
    ...
    def _thread(self) -> None:
        ...
        self.hwnd = user32.CreateWindowExW(
            WS_EX_TOPMOST, "FRANZHUD", "FRANZ System Core",
        ...
        if dwmapi:
            ...
            red_bgr = ctypes.c_int(0x0000FF)
            try:
                dwmapi.DwmSetWindowAttribute(self.hwnd, DWMWA_CAPTION_COLOR, ctypes.byref(red_bgr), ctypes.sizeof(red_bgr))
                dwmapi.DwmSetWindowAttribute(self.hwnd, DWMWA_BORDER_COLOR, ctypes.byref(red_bgr), ctypes.sizeof(red_bgr))
            except Exception:
                pass
        
        mono = gdi32.CreateFontW(-14, 0, 0, 0, 400, 0, 0, 0, 1, 0, 0, 0, 0, "Consolas")
        btn_font = gdi32.CreateFontW(-28, 0, 0, 0, 900, 0, 0, 0, 1, 0, 0, 0, 0, "Segoe UI Black")
        ...
        if self.edit:
            ...
            user32.SendMessageW(self.edit, EM_SETBKGNDCOLOR, 0, 0xFFFFFF)
            user32.SetWindowTextW(self.edit, CONSTITUTION + STARTING_STORY)
            user32.SendMessageW(self.edit, EM_SETZOOM, self.zoom_num, self.zoom_den)
        ...
        if self.btn and btn_font:
            user32.SendMessageW(self.btn, WM_SETFONT, btn_font, 1)
    ...
    def update(self, story: str) -> None:
        if self.edit:
            ...
            user32.SendMessageW(self.edit, EM_GETZOOM, ctypes.cast(ctypes.pointer(current_num), ctypes.c_void_p).value, ctypes.cast(ctypes.pointer(current_den), ctypes.c_void_p).value)
            if current_num.value > 0 and current_den.value > 0:
                self.zoom_num = current_num.value
                self.zoom_den = current_den.value
            
            full_text = CONSTITUTION + story
            user32.SetWindowTextW(self.edit, full_text)
            user32.SendMessageW(self.edit, EM_SETZOOM, self.zoom_num, self.zoom_den)
...
def main() -> None:
    ...
    if STORY_FILE.exists():
        try:
            loaded_story = STORY_FILE.read_text(encoding="utf-8").strip()
            print(f"FRANZ awakens | Loaded memory: {len(loaded_story)} chars")
        except Exception:
            loaded_story = STARTING_STORY
    else:
        loaded_story = STARTING_STORY
        STORY_FILE.parent.mkdir(parents=True, exist_ok=True)
    
    print(f"Screen: {sw}x{sh} | Model: {SCREEN_W}x{SCREEN_H}")
    print(f"Dump: {dump}")
    print("PAUSED - Edit HUD if needed, click RESUME\n")
    
    with HUD() as hud:
        step = 0
        obs: ObsWindow | None = None
        
        if loaded_story != STARTING_STORY:
            hud.update(loaded_story)
        ...
        while not hud.stop.is_set():
            ...
            if hud.hwnd:
                user32.ShowWindow(hud.hwnd, SW_HIDE)
            time.sleep(0.05)
            ...
            if hud.hwnd:
                user32.ShowWindow(hud.hwnd, SW_SHOWNOACTIVATE)
            ...
            try:
                ...
                story_update = str(args.get("story", "")).strip()
                if len(story_update) < 30:
                    print(f"\n[{ts}] {step:03d} | {tool} | ERROR: story too short ({len(story_update)} chars)")
                    time.sleep(1.0)
                    continue
                
                print(f"\n[{ts}] {step:03d} | {tool}\n{story_update[:200]}{'...' if len(story_update) > 200 else ''}\n")
